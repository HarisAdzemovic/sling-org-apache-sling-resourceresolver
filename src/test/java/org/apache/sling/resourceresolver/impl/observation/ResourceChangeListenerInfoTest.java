...
public class ResourceChangeListenerInfoTest {
    ...
    public void testGetExpandedRelativePaths() {
        ...
        when(reference.getProperty(PATHS)).thenReturn(new String[]{ "project/components/page/page.html" });
        ...
        assertTrue(("PathSet " + paths.toString()) + " does not contain /apps/project/components/page/page.html.", paths.contains("/apps/project/components/page/page.html"));
        assertTrue(("PathSet " + paths.toString()) + " does not contain /libs/project/components/page/page.html.", paths.contains("/libs/project/components/page/page.html"));
    }

    @Test
    public void testDotPathConfig() {
        ServiceReference reference = mock(ServiceReference.class);
        when(reference.getProperty(PATHS)).thenReturn(new String[]{ "." });
        final ResourceChangeListenerInfo rcli = new ResourceChangeListenerInfo(reference, SEARCH_PATHS);
        Set<String> paths = rcli.getPaths().toStringSet();
    ...
    public void testGlobPatternExpansion() {
        ServiceReference reference = mock(ServiceReference.class);
        when(reference.getProperty(PATHS)).thenReturn(new String[]{ "glob:./**/*.html" });
        final ResourceChangeListenerInfo rcli = new ResourceChangeListenerInfo(reference, SEARCH_PATHS);
        Set<String> paths = rcli.getPaths().toStringSet();
