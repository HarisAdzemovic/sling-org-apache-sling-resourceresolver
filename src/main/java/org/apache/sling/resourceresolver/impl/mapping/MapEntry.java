...
public class MapEntry implements Comparable<MapEntry> {
    ...
    public static String fixUriPath(final String uriPath) {
        for (int i = 0; i < MapEntry.URL_WITH_PORT_MATCH.length; i++) {
    ...
    public static String toURI(final String uriPath) {
        for (int i = 0; i < MapEntry.PATH_TO_URL_MATCH.length; i++) {
    ...
    public static MapEntry createResolveEntry(String url, final Resource resource, final boolean trailingSlash) {
        ...
        if (props != null) {
            ...
            final String[] internalRedirectProps = props.get(ResourceResolverImpl.PROP_REDIRECT_INTERNAL, String[].class);
    ...
    public static List<MapEntry> createMapEntry(String url, final Resource resource, final boolean trailingSlash) {
        ...
        if (props != null) {
            ...
            if (redirect != null) {
                ...
                LoggerFactory.getLogger(MapEntry.class).info("createMapEntry: Configuration has external redirect to {}; not creating mapping for configuration in {}", redirect, resource.getPath());
                return null;
            }
            // ignore potential regular expression url
            if (isRegExp(url)) {
                LoggerFactory.getLogger(MapEntry.class).info("createMapEntry: URL {} contains a regular expression; not creating mapping for configuration in {}", url, resource.getPath());
                return null;
            }
                ...
                for (final String redir : internalRedirect) {
                    if (!redir.contains("$")) {
                        ...
                        try {
                            ...
                            // ignore this entry
                            LoggerFactory.getLogger(MapEntry.class).debug("Ignoring mapping due to exception: " + iae.getMessage(), iae);
                        }
                        if (mapEntry != null) {
    ...
    public MapEntry(String url, final int status, final boolean trailingSlash, final long order, final String... redirect) {
